...
impl Actor {
    ...
    pub async fn bot_act<'l>(&self, state: &ClearRoomState, arg: Arg<'l>) -> anyhow::Result<()> {
        let neighs = state
            ...
            .collect::<Vec<_>>();
        if self.resources.abilities == 0 || (self.resources.movement == 0 && neighs.is_empty()) {
            ...
            let Some((index, _)) = self
                ...
                .iter()
                .enumerate()
                .filter_map(|(i, id)| DATA.cards.get(id).map(|a| (i, a)))
                .find(|(_, c)| c.unique_id.as_deref() == Some("card_punch"))
            else {
                arg.host.mock(ClientRequest::EndTurn).await?;
                return Ok(());
            };
            arg.host
                .mock(ClientRequest::Action(index, pos.x, pos.y, pos.z))
                .await?
        } else {
            let others = state
                .actors
            ...
            let Some((index, _)) = self
                ...
                .enumerate()
                .filter_map(|(i, id)| DATA.cards.get(id).map(|a| (i, a)))
                .find(|(_, c)| c.unique_id.as_deref() == Some("card_walk"))
            else {
                arg.host.mock(ClientRequest::EndTurn).await?;
                return Ok(());
            };
            arg.host.mock(ClientRequest::Action(index, x, y, z)).await?
        };
        Ok(())
    }
}
