...
#[derive(Clone)]
pub struct Output(channel::Sender<(Vec3, Piece)>);
impl Output {
    pub fn new(tx: channel::Sender<(Vec3, Piece)>) -> Self {
        Self(tx)
    }
    pub fn set(&self, position: Vec3, ground_type: GroundType) {
        self.0
            .send_blocking((position, Piece::Ground(ground_type)))
            .unwrap();
    }
}
...
fn spawn_circle(middle: Vec3, size: usize, ground_type: GroundType, spawn: Output) {
    for x in (middle.x - size)..(middle.x + size) {
        for z in (middle.z - size)..(middle.z + size) {
            ...
            if pos.distance(middle) + 0.2 > size as f64 {
                continue;
            }
            spawn.set(pos, ground_type);
        }
    }
}
