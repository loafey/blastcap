...
fn gen_sparse_floor(rand: &mut Random, y: usize, floor_amount: usize, size: Vec3, spawn: Output) {
    ...
    for (b, _) in boxes.values() {
        for x in b.x1..b.x2 {
            for z in b.z1..b.z2 {
                spawn
                    .send_blocking((Vec3::new(x, y, z), Piece::Ground))
                    .unwrap();
            }
        }
    }
    for (a, _) in boxes.values() {
        for (b, _) in boxes.values() {
            ...
            spawn
                .send_blocking((Vec3::new(middle.x, y, middle.y), Piece::Ground))
                .unwrap();
            for p in a_middle.y.min(b_middle.y)..a_middle.y.max(b_middle.y) {
                spawn
                    .send_blocking((Vec3::new(middle.x, y, p), Piece::Ground))
                    .unwrap();
            }
            for p in a_middle.x.min(b_middle.x)..a_middle.x.max(b_middle.x) {
                spawn
                    .send_blocking((Vec3::new(p, y, middle.y), Piece::Ground))
                    .unwrap();
            }
        }
    }
